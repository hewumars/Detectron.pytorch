...
def smooth_l1_loss(bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights, beta=1.0):
    ...
    smoothL1_sign = (abs_in_box_diff < beta_2).detach().float()
    in_loss_box = smoothL1_sign * 0.5 * torch.pow(in_box_diff, 2) / beta_2 + \
                  (1 - smoothL1_sign) * (abs_in_box_diff - (0.5 * beta_2))
